...
class DashboardState(rx.State):
    ...
    def toggle_notification(self, key: str, value: bool):
        ...
        yield rx.toast(
            "Notification preference updated", position="bottom-right"
        )
    ...
    def clear_all_data(self):
        ...
        yield rx.toast(
            "All expenses have been cleared", position="bottom-right"
        )
    ...
    def ensure_data(self):
        ...
        if not self.expenses:
            ...
            for i in range(50):
                ...
                category = random.choice(cat_names) if cat_names else "Office"
                new_expenses.append(
                    {
                        "id": f"EXP-{1000 + i}",
                        "description": f"Transaction record #{1000 + i}",
            ...
            self.expenses = sorted(
                new_expenses, key=lambda x: x["date"], reverse=True
            )
    ...
    def update_expense(self, form_data: dict):
        ...
        for expense in self.expenses:
            if expense["id"] == expense_id:
                expense["merchant"] = form_data.get(
                ...
                expense["amount"] = float(
                    form_data.get("amount", expense["amount"])
                )
                expense["date"] = form_data.get("date", expense["date"])
                expense["category"] = form_data.get(
    ...
    def export_report(self):
        ...
        yield rx.toast(
            f"Exporting data to {filename}...", position="bottom-right"
        )
    ...
    def category_filter_options(self) -> list[str]:
        return ["All"] + [c["name"] for c in self.categories]

    @rx.var
    def filtered_sorted_expenses(self) -> list[Expense]:
        items = list(self.expenses)
        ...
        try:
            ...
            logging.exception(f"Error sorting expenses: {e}")
        return items
    ...
    def page_count(self) -> int:
        return max(
            1,
            (len(self.filtered_sorted_expenses) + self.limit - 1) // self.limit,
        )
    ...
    def budget_usage_percent(self) -> float:
        if self.total_budget == 0:
            return 0.0
        return min(
            round(self.current_month_expenses / self.total_budget * 100, 1),
            100.0,
        )
    ...
    def chart_trend_data(self) -> list[MonthlyTrend]:
        ...
        if self.chart_range == "This Year":
            ...
            return [
                {"name": m, "amount": round(monthly_map[m], 2)}
    ...
    def report_expenses(self) -> list[Expense]:
        ...
        if self.report_range == "This Week":
            ...
            return [e for e in self.expenses if e["date"] >= start_str]
        elif self.report_range == "This Month":
            month_prefix = now.strftime("%Y-%m")
            return [
                e for e in self.expenses if e["date"].startswith(month_prefix)
            ]
            ...
            return [
                e for e in self.expenses if e["date"].startswith(year_prefix)
            ]
        return list(self.expenses)

    @rx.var
    def report_total(self) -> float:
        return round(sum((e["amount"] for e in self.report_expenses)), 2)
        ...
        return [
            {
                ...
                "value": round(v, 2),
                "fill": color_map.get(
                    cat_colors.get(k, "bg-gray-500"), "#6b7280"
                ),
            }
    ...
    @rx.var
    def report_trend_data(self) -> list[dict[str, str | float]]:
        data = defaultdict(float)
        for e in self.report_expenses:
        ...
        if not sorted_dates:
            return []
        return [
            {"name": d[5:], "amount": round(data[d], 2)} for d in sorted_dates
        ]