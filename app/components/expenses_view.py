...
def form_input(
    ...
    return rx.el.div(
        rx.el.label(
            label, class_name="block text-sm font-medium text-gray-700 mb-1"
        ),
        rx.el.input(
            type=type,
            name=name,
...
def category_select(name: str, default_value: str = "") -> rx.Component:
    return rx.el.div(
        rx.el.label(
            "Category",
            ...
            rx.el.div(
                ...
                rx.el.form(
                    rx.el.div(
                        ...
                        form_input(
                            "Description", "description", "What was this for?"
                        ),
            ...
            rx.el.div(
                ...
                rx.el.form(
                    rx.el.input(
                        type="hidden",
                        name="id",
                        default_value=DashboardState.current_expense["id"],
                    ),
                    rx.el.div(
                        form_input(
                            "Merchant",
...
def table_header_cell(label: str, sort_key: str = "") -> rx.Component:
    if not sort_key:
        return rx.el.th(
            rx.el.div(label, class_name="flex items-center gap-2 select-none"),
    ...
    return rx.el.th(
        ...
        class_name="text-left text-xs font-semibold text-gray-500 pb-4 pl-4 uppercase tracking-wider",
        on_click=DashboardState.toggle_sort(sort_key),
    )
...
def expense_row(expense: Expense) -> rx.Component:
    return rx.el.tr(
        ...
        rx.el.td(
            rx.el.span(
                expense["date"], class_name="text-sm text-gray-600 font-medium"
            ),
            class_name="px-3 py-4 whitespace-nowrap",
        ),
        rx.el.td(
...
def expenses_view() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.div(
                rx.el.h2(
                    "Expenses", class_name="text-xl font-bold text-gray-900"
                ),
                rx.el.p(
            ...
            rx.el.div(
                ...
                rx.el.select(
                    rx.foreach(
                        DashboardState.category_filter_options,
                        lambda opt: rx.el.option(opt, value=opt),
                    ),
        ...
        rx.el.div(
            rx.el.div(
                rx.el.table(
                    ...
                    rx.el.tbody(
                        rx.cond(
                            DashboardState.paginated_expenses.length() > 0,
                            rx.foreach(
                                DashboardState.paginated_expenses, expense_row
                            ),
                            rx.el.tr(
                                rx.el.td(
                                    rx.el.div(
