...
def report_stat_card(
    label: str, value: str, icon: str, color: str
) -> rx.Component:
    color_classes = {
    ...
    text_cls, bg_cls = color_classes.get(color, color_classes["blue"])
    return rx.el.div(
        rx.el.div(
        ...
        rx.el.div(
            rx.icon(icon, size=24, class_name=text_cls),
            class_name=f"w-12 h-12 rounded-xl {bg_cls} flex items-center justify-center",
        ),
...
def spending_trend_chart() -> rx.Component:
    return rx.el.div(
        rx.el.h3(
            "Spending Trend", class_name="text-lg font-bold text-gray-900 mb-6"
        ),
        rx.el.div(
...
def insights_section() -> rx.Component:
    return rx.el.div(
        rx.el.h3(
            "AI Insights", class_name="text-lg font-bold text-gray-900 mb-4"
        ),
        rx.el.div(
            rx.el.div(
                rx.icon(
                    "trending-up", size=20, class_name="text-amber-600 mt-1"
                ),
                rx.el.div(
                    rx.el.p(
                        "Spending Alert",
            ...
            rx.el.div(
                rx.icon(
                    "circle-check", size=20, class_name="text-emerald-600 mt-1"
                ),
                rx.el.div(
                    rx.el.p(
                        "Budget on Track",
...
def reports_view() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.div(
                rx.el.h2(
                    "Reports", class_name="text-xl font-bold text-gray-900"
                ),
                rx.el.p(
            ...
            rx.el.div(
                rx.el.select(
                    rx.el.option("This Week", value="This Week"),
                    rx.el.option("This Month", value="This Month"),
                    rx.el.option("Last Month", value="Last Month"),
