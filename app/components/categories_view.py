...
def category_card(category: Category) -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.div(
                rx.icon("tag", class_name="text-white", size=20),
                class_name=rx.cond(
                    category["color"] == "bg-blue-500",
                    "w-10 h-10 rounded-xl bg-blue-500 flex items-center justify-center shadow-md opacity-90",
                    rx.cond(
                        category["color"] == "bg-purple-500",
                        "w-10 h-10 rounded-xl bg-purple-500 flex items-center justify-center shadow-md opacity-90",
                        rx.cond(
                            category["color"] == "bg-green-500",
                            "w-10 h-10 rounded-xl bg-green-500 flex items-center justify-center shadow-md opacity-90",
                            rx.cond(
                                category["color"] == "bg-orange-500",
                                "w-10 h-10 rounded-xl bg-orange-500 flex items-center justify-center shadow-md opacity-90",
                                rx.cond(
                                    category["color"] == "bg-teal-500",
                                    "w-10 h-10 rounded-xl bg-teal-500 flex items-center justify-center shadow-md opacity-90",
                                    "w-10 h-10 rounded-xl bg-gray-500 flex items-center justify-center shadow-md opacity-90",
                                ),
                            ),
                        ),
                    ),
            ...
            rx.el.div(
                rx.el.button(
                    ...
                    on_click=lambda: DashboardState.open_cat_edit_modal(
                        category
                    ),
                ),
                rx.el.button(
                    ...
                    on_click=lambda: DashboardState.open_cat_delete_modal(
                        category
                    ),
                ),
        ...
        rx.el.h3(
            category["name"], class_name="text-lg font-bold text-gray-900 mb-1"
        ),
        rx.el.p(
            "Budget Limit: $" + category["budget"].to_string(),
...
def category_form_dialog(submit_handler, is_edit: bool = False) -> rx.Component:
    return rx.el.form(
        rx.cond(
            is_edit,
            rx.el.input(
                type="hidden",
                name="id",
                default_value=DashboardState.current_category["id"],
            ),
            rx.fragment(),
        ),
        rx.el.div(
            ...
            rx.el.div(
                ...
                rx.el.input(
                    ...
                    name="budget",
                    placeholder="0.00",
                    default_value=DashboardState.current_category[
                        "budget"
                    ].to_string(),
                    class_name="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 outline-none transition-all text-sm",
                    required=True,
        ...
        key=DashboardState.current_category["id"],
    )
...
def add_category_dialog() -> rx.Component:
    return rx.dialog.root(
        rx.dialog.content(
            ...
            category_form_dialog(DashboardState.add_category, is_edit=False),
...
def edit_category_dialog() -> rx.Component:
    return rx.dialog.root(
        rx.dialog.content(
            ...
            category_form_dialog(DashboardState.update_category, is_edit=True),
            class_name="bg-white p-6 rounded-2xl shadow-xl max-w-md w-full outline-none",
        ),
...
def delete_category_dialog() -> rx.Component:
    return rx.dialog.root(
        rx.dialog.content(
            rx.el.div(
                rx.icon(
                    "triangle-alert", class_name="text-red-500 mb-4", size=48
                ),
                rx.el.h2(
                    "Delete Category?",
...
def categories_view() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.div(
                rx.el.h2(
                    "Categories", class_name="text-xl font-bold text-gray-900"
                ),
                rx.el.p(
